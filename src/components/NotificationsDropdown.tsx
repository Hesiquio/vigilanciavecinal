...
export function NotificationsDropdown() {
  ...
  const unreadActivity: ActivityItem[] = useMemo(() => {
    ...
    return [...typedAlerts, ...typedAvisos]
      .filter(item => item.timestamp && !readNotificationIds.has(item.id))
  ...
  const renderContent = () => {
    ...
    return unreadActivity.map((item) => {
        return (
  ...
  return (
    <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-80 md:w-96">
            <DropdownMenuLabel>Notificaciones Nuevas</DropdownMenuLabel>
            <DropdownMenuSeparator />
